...
(:predicates
    ...
    (incapacitated ?char - character)
)
(:action move
    :parameters (?char - character ?from ?to - location)
    :precondition (and (atLoc ?char ?from) (not (isDead ?char)) (not (incapacitated ?char)) (not (unaware ?char ?to)) (not (= ?from ?to)) (not (cantMove ?char)) (or (isConnected ?from ?to) (isConnected ?to ?from))
    )
...
(:action slay
    ...
    (forall (?vict - character) (when (isSwallowed ?vict ?mon) (and (isSaved ?vict) (not (isSwallowed ?vict ?mon)) (not (incapacitated ?vict)) )))
    )
)
(:action askInfo
...
(:action swallow
    :parameters (?mon ?vict - character ?loc - location)
    ...
    :effect (and (isSwallowed ?vict ?mon) (cantMove ?mon) (incapacitated ?vict)
    )
)
)